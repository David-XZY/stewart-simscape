...
if strlength(resultFile) == 0
    candidates = dir(fullfile(projectRoot, 'results', 'controller', 'pwm_pose_force_comparison_*.mat'));
    [~, index] = max([candidates.datenum]);
    resultFile = string(fullfile(candidates(index).folder, candidates(index).name));
end
if strlength(outputDir) == 0
    outputDir = fullfile(projectRoot, 'results', 'reports', 'ukf_visuals_20260615');
end
