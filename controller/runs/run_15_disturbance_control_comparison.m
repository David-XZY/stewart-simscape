...
strictOverrides.dt = config.sampleTime;
strictConfig = makeStrictClfCbfQpConfig(setup.model, strictOverrides);
warmStrictQp(nominalReference, setup, strictConfig);
...
baselineControllers = [makeController("fixed_lqi", fixedSchedule, false, false, defaultDob); ...
...
candidateControllers = [ ...
    makeController("scheduled_lqi", scheduledSchedule, false, false, bestDob); ...
    makeController("fixed_lqi_dob", fixedSchedule, true, false, bestDob); ...
    makeController("scheduled_lqi_dob", scheduledSchedule, true, false, bestDob); ...
    makeController("scheduled_lqi_dob_strict_qp", scheduledSchedule, true, true, bestDob)];
...
function controller = makeController(id, schedule, useDob, useStrictQp, dobConfig)
...
function warmStrictQp(reference, setup, strictConfig)
...
for index = 1:3
    stepStrictClfCbfQp(reference.q(:, 1), reference.qd(:, 1), ...
        ...
        previous, previous, setup.model, setup.scene, strictConfig);
end
end
