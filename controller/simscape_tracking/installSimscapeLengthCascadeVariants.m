...
jacobianReference = [referenceBlock, '/Reference Jacobian'];
if ~blockExists(jacobianReference)
    add_block('simulink/Sources/From Workspace', jacobianReference, ...
        'VariableName', 'references.rJq', ...
        'Position', [55 225 170 255]);
end
set_param([referenceBlock, '/Bus Creator'], 'Inputs', '4');
jacobianLine = get_param(jacobianReference, 'LineHandles');
if jacobianLine.Outport == -1
    jacobianLine.Outport = add_line(referenceBlock, ...
        'Reference Jacobian/1', 'Bus Creator/4');
end
set_param(jacobianLine.Outport, 'Name', 'rJq');
...
add_block('simulink/Signal Routing/Bus Selector', [target, '/Reference Signals'], ...
    'OutputSignals', 'rL,rLd,r,rJq', 'Position', [90 195 135 295]);
add_block('simulink/Math Operations/Sum', [target, '/Pose Error'], ...
    'Inputs', '+-', 'Position', [175 245 200 275]);
add_block('simulink/Discrete/Discrete Transfer Fcn', [target, '/Pose Error Lowpass'], ...
    'Numerator', 'poseLengthDesign.poseFeedbackFilterNumerator', ...
    'Denominator', 'poseLengthDesign.poseFeedbackFilterDenominator', ...
    'SampleTime', 'poseLengthConfig.sampleTime', ...
    'Position', [230 240 335 280]);
add_block('simulink/Math Operations/Product', [target, '/Reference Jacobian Product'], ...
    'Inputs', '**', 'Multiplication', 'Matrix(*)', ...
    'Position', [375 225 440 290]);
add_block('simulink/Math Operations/Gain', [target, '/Pose Feedback Gain'], ...
    'Gain', 'poseLengthDesign.poseFeedbackGain', ...
    'Position', [475 240 565 280]);
add_block('simulink/Discontinuities/Saturation', [target, '/Pose Correction Limit'], ...
    ...
    'Position', [600 235 695 285]);
add_block('simulink/Math Operations/Sum', [target, '/Corrected Length Reference'], ...
    'Inputs', '++', 'Position', [730 175 755 205]);
add_block('simulink/Math Operations/Sum', [target, '/Position Error'], ...
    'Inputs', '+-', 'Position', [795 115 820 145]);
add_block('simulink/Math Operations/Gain', [target, '/Position P'], ...
    'Gain', 'diag(poseLengthDesign.Kpos)', ...
    'Multiplication', 'Matrix(K*u)', 'Position', [855 110 945 150]);
add_block('simulink/Math Operations/Sum', [target, '/Speed Command'], ...
    'Inputs', '++', 'Position', [985 165 1010 195]);
add_block('simulink/Discontinuities/Saturation', [target, '/Speed Limit'], ...
    'UpperLimit', 'poseLengthConfig.speedLimit', ...
    'LowerLimit', '-poseLengthConfig.speedLimit', ...
    'Position', [1045 160 1125 200]);
add_block('simulink/Discontinuities/Rate Limiter', [target, '/Acceleration Limit'], ...
    'RisingSlewLimit', 'poseLengthConfig.accelerationLimit', ...
    'FallingSlewLimit', '-poseLengthConfig.accelerationLimit', ...
    'InitialCondition', 'zeros(6,1)', 'Position', [1160 160 1250 200]);
add_block('simulink/Math Operations/Sum', [target, '/Speed Error'], ...
    'Inputs', '+-', 'Position', [1290 105 1315 135]);
add_block('simulink/Discrete/Discrete PID Controller', [target, '/Velocity PIDF'], ...
    'Controller', 'PID', 'TimeDomain', 'Discrete-time', ...
    ...
    'Kt', '1/poseLengthConfig.sampleTime', 'Position', [1355 95 1455 145]);
add_block('simulink/Discontinuities/Rate Limiter', [target, '/Servo Acceleration Limit'], ...
    'RisingSlewLimit', 'poseLengthConfig.accelerationLimit', ...
    'FallingSlewLimit', '-poseLengthConfig.accelerationLimit', ...
    'InitialCondition', 'zeros(6,1)', 'Position', [1470 95 1545 145]);
add_block('simulink/Ports & Subsystems/Out1', [target, '/u'], ...
    'Port', '1', 'Position', [1580 110 1610 130]);
...
add_line(target, 'Pose Error/1', 'Pose Error Lowpass/1');
add_line(target, 'Reference Signals/4', 'Reference Jacobian Product/1');
add_line(target, 'Pose Error Lowpass/1', 'Reference Jacobian Product/2');
add_line(target, 'Reference Jacobian Product/1', 'Pose Feedback Gain/1');
add_line(target, 'Pose Feedback Gain/1', 'Pose Correction Limit/1');
