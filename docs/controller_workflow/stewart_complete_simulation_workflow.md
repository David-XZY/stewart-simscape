...
| 单条支链模型 | `matlab/simscape_subsystems/stewart_strut.slx` | 关节、杆件、Prismatic Joint 和执行器 Variant |
| 理想力位姿控制 | `controller/runs/run_02_simscape_length_control.m` | Run02 |
| 理想腿长串级控制 | `controller/runs/run_03_simscape_length_cascade_control.m` | Run03 |
| 位姿反馈理想腿长控制 | `controller/runs/run_04_simscape_pose_length_control.m` | Run04 |
| PWM 辨识数据 | `controller/runs/run_05_generate_pwm_identification_data.m` | 生成训练、验证和测试数据 |
| 灰箱加 NARX 训练 | `controller/runs/run_06_train_pwm_force_identifier.m` | 训练和闭环精炼 |
| PWM 双线闭环 | `controller/runs/run_07_compare_pwm_pose_force_control.m` | Oracle 与辨识反馈对比 |
...
单支链模型 `matlab/simscape_subsystems/stewart_strut.slx` 包含：
...
```text
docs/controller_workflow/figures/simulation_workflow/
```
