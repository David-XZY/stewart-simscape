...
Simscape 验证入口为 `run_02_simscape_length_control.m`，当前采用 IHSID 力前馈加长度误差反馈。
...
当前自动整定候选为 `0.5/0.25/0.125 Hz`。线性化整定时会临时清零 `uFF`，确保反馈
对象不被已知前馈偏置污染；完整仿真时再恢复 IHSID 前馈力。脚本会保存腿长、位姿、
反馈力、前馈力、总控制力和优化力对照诊断。
