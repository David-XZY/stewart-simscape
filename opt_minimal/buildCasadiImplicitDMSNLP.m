function nlpData = buildCasadiImplicitDMSNLP(model, scene, disc, initialGuess, solverOptions)
% buildCasadiImplicitDMSNLP - 构建 DMSID 直接多重射击隐式动力学 NLP
%
% 文件用途：
%   构造运动圆柱体-固定长方体两阶段轨迹优化问题，保留 Stewart 隐式动力学、
%   直接多重射击转录、CasADi/IPOPT/MA27 和统一目标函数。
%
% 输入参数：
...
%
% 核心公式：
%   r_shoot=X_{k+1}-Phi_k(X_k,F_k,F_mid,F_{k+1})；
%   r_dyn=Jv(q)'F-Wreq(q,qd,qdd)；
%   d_box=-h_z-(z_C^B+rho_z)。
%
% 在优化链路中的作用：
%   run_04_compare_CHSID_IHSID_DMSID 调用本函数构建 DMSID 分支。
import casadi.*
if nargin < 4 || ~isfield(initialGuess, 'nominalStage1')
    error('buildCasadiImplicitDMSNLP:MissingNominalReference', ...
...
for intervalIndex = 1:disc.numIntervals
    Xc = rk4IntegrateImplicitA(Xnode(:, intervalIndex), Anode(:, intervalIndex), ...
        Amid(:, intervalIndex), Anode(:, intervalIndex+1), 0, 0.5, disc.h);
    XendProp = rk4IntegrateImplicitA(Xc, Anode(:, intervalIndex), ...
        Amid(:, intervalIndex), Anode(:, intervalIndex+1), 0.5, 1.0, disc.h);
...
gEq = vertcat(gShoot{:}, gDyn{:}, gWaypoint, gStage2{:});
...
nlpData.J = J;
nlpData.eval = Function('implicit_dms_eval', {z}, {J, gEq, cIneqExpr}, ...
    {'z'}, {'J', 'gEq', 'cIneq'});
nlpData.sizes = sizes;
nlpData.opts = opts;
nlpData.method = 'DMSID';
...
% rk4IntegrateImplicitA - DMSID 区间内以二次插值 A(t) 积分状态
%
% 输入：
%   X0              - 子步起点状态 [q;qd]。
%   Aleft/Amid/Aright - 区间左端、中点、右端的广义加速度优化变量。
%   tau0/tau1      - 归一化区间坐标。
%   h              - 物理区间长度。
%
% 输出：
%   X1 - 按 f=[qd;A(t)] RK4 积分得到的子步终点状态。
dt = h * (tau1 - tau0);
f1 = dmsidSlope(X0, Aleft, Amid, Aright, tau0);
f2 = dmsidSlope(X0 + 0.5*dt*f1, Aleft, Amid, Aright, 0.5*(tau0+tau1));
f3 = dmsidSlope(X0 + 0.5*dt*f2, Aleft, Amid, Aright, 0.5*(tau0+tau1));
f4 = dmsidSlope(X0 + dt*f3, Aleft, Amid, Aright, tau1);
X1 = X0 + dt/6*(f1 + 2*f2 + 2*f3 + f4);
end

function slope = dmsidSlope(X, Aleft, Amid, Aright, tau)
A = quadraticAccelExpr(Aleft, Amid, Aright, tau);
slope = [X(7:12); A];
end

function A = quadraticAccelExpr(Aleft, Amid, Aright, tau)
