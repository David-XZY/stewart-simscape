function nlpData = buildCasadiImplicitIHSNLP(model, scene, disc, initialGuess, solverOptions)
% buildCasadiImplicitIHSNLP - 构建两阶段 standard IHSID 轨迹优化 NLP
%
% 文件用途：
...
%
% 在优化链路中的作用：
%   run_01_ihsid_trajectory 调用本函数获得默认 IPOPT/MA27 求解器。
import casadi.*
if nargin < 4 || ~isfield(initialGuess, 'nominalStage1')
    error('buildCasadiImplicitIHSNLP:MissingNominalReference', ...
        '必须传入 buildInitialGuessTwoPhaseIHSImplicit 生成的 initialGuess.nominalStage1 作为第一阶段标称轨迹。');
end
if nargin < 5
...
[Xnode, Xmid, Anode, Amid, Fnode, Fmid, separator] = unpackSymbolic(z, scene, disc);
...
for nodeIndex = 1:disc.numNodes
    point = pointExpressions(Xnode(:, nodeIndex), Fnode(:, nodeIndex), Anode(:, nodeIndex), model);
...
for intervalIndex = 1:disc.numIntervals
    ...
    gMid{end+1} = Xc - 0.5*(Xnode(:, intervalIndex) + Xnode(:, intervalIndex+1)) - ...
        disc.h/8*(fNode(:, intervalIndex) - fNode(:, intervalIndex+1)); %#ok<AGROW>
    pointMid = pointExpressions(Xc, Fmid(:, intervalIndex), Amid(:, intervalIndex), model);
...
gEq = vertcat(gMid{:}, gHS{:}, gDyn{:}, gWaypoint, gStage2{:});
...
solver = nlpsol('ihsid_ipopt_solver', 'ipopt', nlp, opts);
...
nlpData.gMidConsistency = vertcat(gMid{:});
nlpData.J = J;
nlpData.eval = Function('implicit_ihs_eval', {z}, {J, gEq, cIneqExpr}, ...
    {'z'}, {'J', 'gEq', 'cIneq'});
nlpData.sizes = sizes;
nlpData.opts = opts;
nlpData.method = 'IHSID';
nlpData.solverBackend = 'ipopt';
end
...
sizes = struct();
sizes.numZ = 12*(numNodes - 2) + 12*numMidpoints + 6*numNodes + ...
    6*numMidpoints + 6*numNodes + 6*numMidpoints;
numCollisionCertificates = getNumCollisionCertificates(disc);
sizes.numZ = sizes.numZ + 8*numCollisionCertificates;
sizes.numEq = 24*numIntervals + 6*(numNodes + numMidpoints) + 6 + ...
...
function [Xnode, Xmid, Anode, Amid, Fnode, Fmid, separator] = unpackSymbolic(z, scene, disc)
cursor = 0;
...
nodeForceCount = 6*disc.numNodes;
midForceCount = 6*disc.numMidpoints;
totalLength = internalCount + midStateCount + nodeAccelCount + midAccelCount + nodeForceCount + midForceCount;
numCollisionCertificates = getNumCollisionCertificates(disc);
totalLength = totalLength + 8*numCollisionCertificates;
lbz = -inf(totalLength, 1);
ubz = inf(totalLength, 1);
forceStart = internalCount + midStateCount + nodeAccelCount + midAccelCount + 1;
...
function point = pointExpressions(X, F, A, model)
q = X(1:6);
qd = X(7:12);
kin = ikExpr(q, model);
[Jv, Jq] = jacobianExpr(q, kin);
Ld = Jq * qd;
Ldd = legAccelExpr(q, qd, A, kin, Jq);
