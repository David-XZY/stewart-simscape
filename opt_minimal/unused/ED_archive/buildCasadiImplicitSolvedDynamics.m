...
%
% 输入：
%   model - Stewart 平台几何、动力学和数值阈值结构体。
%
% 输出：
%   dynFun - CasADi Function，输入 X=[q;qd]、F，输出 xdot=[qd;A]、A、rDyn。
%
% 公式：
%   Wbias = Wreq(q,qd,0)
%   H(:,i) = Wreq(q,qd,e_i) - Wbias
%   A = solve(H, Jv(q).'*F - Wbias)
%   rDyn = Jv(q).'*F - Wreq(q,qd,A)
%
% 在实验链路中的作用：
%   CHSED 与 DMSED 共用该物理动力学求值；A 被消去，但原隐式平衡方程不改变。
import casadi.*
X = MX.sym('X', 12, 1);
F = MX.sym('F', 6, 1);
q = X(1:6);
